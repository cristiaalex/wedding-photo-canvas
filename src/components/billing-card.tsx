...
export function BillingCard({ eventId }: { eventId?: string }) {
  ...
  if (state?.hasPurchase) return <div className="border border-gold/40 bg-champagne/30 p-6"><p className="flex items-center gap-2 text-display text-2xl"><Check className="h-5 w-5 text-gold" /> Artwork purchased</p><p className="mt-3 text-sm leading-6 text-muted-foreground">Your final print-ready mosaic will appear here as soon as it is finished.</p>{state.hasCustomer && <Button variant="outline" className="mt-5" onClick={() => void portal()} disabled={busy !== null}>{busy === "portal" ? <Loader2 className="animate-spin" /> : <CreditCard />}Payment receipt</Button>}</div>;

  return <div className="border border-border bg-card p-6 md:p-8">
    <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-eyebrow text-gold">Your final mosaic</p><h3 className="mt-2 text-display text-3xl">{PET_PRICE_LABEL}</h3><p className="mt-2 text-sm text-muted-foreground">One payment · High-resolution final artwork</p></div><Button size="lg" onClick={() => void checkout()} disabled={busy !== null}>{busy === "checkout" ? <Loader2 className="animate-spin" /> : <CreditCard />}Purchase final mosaic</Button></div>
