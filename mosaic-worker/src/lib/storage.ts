...
export async function uploadVariant(opts: {
  ...
  if (error) {
    throw new Error(
      `Storage upload (${opts.variant}) failed: ${error.message}`,
    );
  }
...
export async function createSignedUrl(
  ...
  if (error) {
    throw new Error(`Sign url failed: ${error.message}`);
  }
...
export async function uploadVariantResumable(opts: {
  ...
  // Use the direct storage hostname rather than the project API hostname.
  ...
  await new Promise<void>((resolve, reject) => {
    const upload = new tus.Upload(opts.body as unknown as Buffer, {
      ...
      headers: {
        // Supabase secret keys are API keys, not JWT access tokens.
